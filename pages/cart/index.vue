        ...
        <el-row :gutter="12">
                  ...
                  >
                    <template #checkbox>
                      <div class="h-100 d-flex align-items-center me-3">
                        <el-checkbox v-model="product.checkBuy" @change="handleChangeProduct(index)" :disabled="isPayOrder"></el-checkbox>
                      </div>
                    </template>
                  </CartProduct>
                  <hr class="my-3">
          ...
          <el-col :xl="9" :lg="10" :md="12" :sm="24">
            <el-card>
              ...
              <div v-if="isPayOrder">
                <h6 class="mb-1 text-primary">Quét mã QrCode dưới đây để hoàn tất đơn hàng</h6>
                <img
                  :src="qrImage"
                  alt="QRCODE"
...
import {getAccessToken, getLocation, getSessionCart, getUserInfo, setOrderCard} from "@/utils/cookieAuthen";
...
export default {
  ...
  methods: {
    ...mapActions('cart', {
      ...
      apiGetCartUsers: 'apiGetCartUsers',
      apiAddProductCart: 'apiAddProductCart',
      apiCreateOder: 'apiCreateOder',
    }),
    ...mapActions('payment', {
    ...
    async getListProduct() {
      try {
        ...
        if (this.user && token) {
          await this.apiGetCartUsers(this.user.id).then((res) => {
    ...
    async handleIncreaseProduct(product, index) {
      if (this.user) {
        const payload = {
          userId: this.user.id,
          productVariantId: product.product.productVariants[0].id,
          quantity: 1
        }
        await this.apiAddProductCart(payload).then((res) => {
          if (res !== undefined) {
            this.$message({
    ...
    async handleReductionProduct(product, index) {
      if (this.products[index].quantity < 2) return
      if (this.user) {
        const payload = {
          userId: this.user.id,
          productVariantId: product.product.productVariants[0].id,
    ...
    async handleOrder() {
      try {
        ...
        this.productsBuy.forEach((item) => {
          orderDetails.push({
            productVariantId: item.product.productVariants[0].id,
        ...
        this.apiCreateOder(payload).then((res) => {
          if (res !== undefined) {
    ...
    async getQrCode(payload) {
      await this.apiGenerateQRCode(payload).then((res) => {
        if (res !== undefined) {
          this.qrImage = res
        }
      })
